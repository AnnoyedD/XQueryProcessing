		...
		if (xqParent != null) {
			...
			if (ctxIndex != -1) {
				...
				if (ctxIndex >= 3 && xqParent.getChild(ctxIndex - 3).getText().equals("for")) {
					...
					System.out.println("stack size of "+var+" is "+stack.size());
					System.out.println("size of arr 1 is "+stack.get(0).size());
				...
				} else if (ctxIndex >= 0 && ctxIndex< xqParent.getChildCount() 
						&& (xqParent.getChild(ctxIndex + 1).getText().equals("//")
							||xqParent.getChild(ctxIndex + 1).getText().equals("/")
						)
						) {
					...
					System.out.println("// VarBind find var ");
